...
/***************Slider for Testimonial***************/
  ...
  /***************Slider for Project detail***************/
      ...
      iframe: {
          ...
          patterns: {
              bilibili: {
                  ...
                  id: '?aid=',
                  src: 'https://player.bilibili.com/player.html?aid=%id%&bvid=%bvid%&cid=%cid%&p=1'
              }
          },
    ...
    $('.popup-tableau').magnificPopup({
      type: 'inline',
      removalDelay: 300, // 设置延迟时间，以便在关闭弹出窗口后保留Tableau视图
      mainClass: 'mfp-fade', // 添加淡入淡出动画
      preloader: false, // 禁用预加载器
      midClick: true, // 允许通过中间点击打开链接
      removalDelay: 300, // 设置延迟时间，以便在关闭弹出窗口后保留Tableau视图
      callbacks: {
          open: function() {
              // 打开弹出窗口时，确保Tableau视图可见
              $('#tableauContainer').css('display', 'block');
          },
          close: function() {
              // 关闭弹出窗口时，隐藏Tableau视图
              $('#tableauContainer').css('display', 'none');
          }
      }
  });
  ...
  /***************For About Us Counter Script***************/
    ...
    $('.rhw-navbar-nav').on('click', 'li', function() {
      $('.rhw-navbar-nav li.active').removeClass('active');
